...
def tune():
    ...
    # call the tuner
    results, env = kt.tune_kernel("calculate_temp", "hotspot_tuned.cu", problem_size, args, tune_params,
                                  metrics=metrics, grid_div_x=grid_div_x, grid_div_y=grid_div_y,
                                  restrictions=restrictions, verbose=True)
                            
    store_results("calculate_temp.json","calculate_temp","hotspot_tuned.cu",tune_params,problem_size, results, env, top=3, objective="GFLOP/s")
    create_device_targets("calculate_temp.h","calculate_temp.json", objective="GFLOP/s")
